...
After analyzing the program, we know the above 2 errors. In addition to the info functionn it performs `write(1, &gbuf, 0x20)`, which means it will take the `0x20` bytes stored in `gbuf` and print out them. If we can overwrite `gbuf` so that its first eight bytes hold a libc address, then calling `info` function will spit out that address.
